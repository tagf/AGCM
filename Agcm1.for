...
C
      ...
      SEAT2(I) = SS(I,J,IPT2)
C
      IF(SEAT1(I).LE.200. ) SEAT1(I) = SEAT1(I)+TICE
      IF(SEAT2(I).LE.200. ) SEAT2(I) = SEAT2(I)+TICE
   70 CONTINUE
      DO 80 I=2,73
      SEAVAL(I) = (SEAT2(I)-SEAT1(I))*FACTOR+SEAT1(I)
   80 CONTINUE
...
C***********************
	do i=2,73
       ...
       if (ISFTYP(I,J).eq.7.or.ISFTYP(I,J).eq.9) then
        IF (IPEROD.LT.IHMN(IARY).AND.SS(I,J,IPT1).GT.200.) !3
     1   SETICE(I,J) = .TRUE.
        IF (IPEROD.GE.IHMN(IARY).AND.SS(I,J,IPT2).GT.200.)  !3
     1   SETICE(I,J) = .TRUE.
